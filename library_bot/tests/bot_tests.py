...
class TestBot(TestCase):
    ...
    def test_borrowing_overdue_check(self, mock_get_users, mock_send_message, mock_notification):
        ...
        borrowing = Borrowing.objects.create(
            borrow_date=timezone.now().date() - timezone.timedelta(days=2),
        ...
        print(Borrowing.objects.select_related("book", "user").filter(expected_return_date__lte=timezone.now().date()))
